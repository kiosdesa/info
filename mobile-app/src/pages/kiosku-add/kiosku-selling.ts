...
@IonicPage()
@Component({
  selector: 'page-kiosku-selling',
  templateUrl: 'kiosku-selling.html',
  providers: [LowerCasePipe]
})
export class KioskuSellingPage
{
    ...
    public navParams: NavParams) 
  {
    ...
    await this.api.post(this._cluster, endpoint, {init:'seller-self', pack:{filedForm:{planning:'create'}}})
    .subscribe((res:any) => {
  ...
  async doCreate()
  {
    ...
    loader.present();
    await this.api
    .post(this._cluster, endpoint, {init:'seller-addnew', pack:{fieldForm:{trace:this._dataKios}}})
    .subscribe((res: any) => {
  ...
  sanitizeInput(val, index)
  {
    ...
    const regexSanitize = this._dataKios[index].field == 'slug' ? regexSanitizeIII : (
      this._dataKios[index].field == 'name' ? regexSanitizeI : regexSanitizeII
    );
    val = this._dataKios[index].field == 'slug' ? this.lowcasePipe.transform(val) : val;
